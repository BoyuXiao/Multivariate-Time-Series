...
//     vector<double>k_th = {0.85,0.8,0.75,0.65,0.6};
//
//     clock_t start;
//     clock_t end;
//     clock_t lb_mv_time;
...
//         double kth = k_th[w];
//
//         int count_p=0,count_i=0,count_m=0;
//
//
//
//       //   // -------------------------------------------------------------------------
//         vector<double> threshold(n1);
//         vector<double> threshold_2(n1);
//         if (Time_DTW_TEST) {
//             // start = clock();
//             for (int i = 0; i < n1; ++i) {
//                 vector<double> dtw_values;
//                 for (int j = 0; j < n2; ++j) {
//                     // cout<<dtw(Test[i], Train[j], K, m, r)<<endl;
//                     double d = dtw(Test[i], Train[j], K, m, r);
//                     // cout<<d<<endl;
//                     dtw_values.push_back(d);
//                 }
//                 // cout<<endl<<endl;
//                 sort(dtw_values.begin(), dtw_values.end(),greater<double>());
//                 size_t total_size = dtw_values.size();
//                 size_t top_count = static_cast<size_t>(std::ceil(total_size * 1));
//
//                 double sum = 0.0;
...
//
//         for (int i = 0; i < n1; ++i) {
//             threshold_2[i] = threshold[i]*threshold[i];
//         }
//       //
//         if (Time_MV_TEST) {
//             lb_mv_time = 0;
...
//                         for (int t=0;t<m;t++) {
//                             double lb_temp = 0;
//                             if (new_train[k][t]<Q_L[k][t]) {
//                                 lb_temp = dist(new_train[k][t],Q_L[k][t]);
//                             }else if (new_train[k][t]>Q_U[k][t]) {
//                                 lb_temp = dist(new_train[k][t],Q_U[k][t]);
//                             }
//                             lb_mv+=lb_temp;
//                             if (lb_mv>threshold_2[i]) {break;}
//                         }
//                     }
//                     if (lb_mv<threshold_2[i]) {
//                         dtw(Test[i], Train[j], K, m, r,threshold_2[i]);
//                         // count_m++;
//                     }
//                     end = clock();
//                     lb_mv_time += (end - start);
...
//                           }
//                           lb_mv+=lb_temp;
//                           if (lb_mv>threshold_2[i]) {flag=1;break;}
//                       }
//                       if (flag)break;
//                   }
//                   if (lb_mv<threshold_2[i]) {
//                       for (int t=0;t<m;t++) {
//                           const int start_ = MAX(0,t-new_r);
//                           const int end_ = MIN(m-1,t+new_r);
//                           double min_val = MAX_VAL;
//                           for (int jj=start_;jj<=end_;jj++) {
//                               double distance = euclideanDistance_2(Test[i][t],Train[j][jj]) - lb_record[jj];
//                               min_val = MIN(min_val,distance);
//                           }
//                           lb_mv+=MAX(0.0,min_val);
//                           if (lb_mv>threshold_2[i]) {break;}
//                       }
//                       if (lb_mv<threshold_2[i]) {
//                           dtw(Test[i], Train[j], K, m, r,threshold_2[i]);
...
//                     start = clock();
//                     int flag = 0;
//                     LB_P_MV_early(new_train,new_test,Q_L,Q_U,new_r,threshold[i],flag,lb_p);
//                     if (flag)continue;
//                     if (lb_p<threshold_2[i]) {
//                         dtw(Test[i], Train[j], K, m, r,threshold_2[i]);
//                         // count_p++;
//                     }
//                     end = clock();
...
//             start = clock();
//             for (int i = 0; i < n1; ++i) {
//                 DTWDistanceWindowLB_Ordered_LBTI(Test[i],Train,r,threshold[i],5);
//             }
//             end = clock();
//             Time_TI = static_cast<double>(end - start) / CLOCKS_PER_SEC;
//         }
//
//
//
...
//             start = clock();
//             for (int i = 0; i < n1; ++i) {
//                 DTWDistanceWindowLB_Ordered_LBPC_(6,2,Test[i],Train,r);
//             }
//             end = clock();
//             Time_PC = static_cast<double>(end - start) / CLOCKS_PER_SEC;
//         }
//
//         cout<<count_m<<" "<<count_i<<" "<<count_p<<endl;
//
//
//         //统计结果------------------------------------------------------------
//         outputFile.open("..//output.txt",std::ios::app);
//         outputFile << name <<"\t"<<Time_DTW<< "\t" <<Time_MV  << "\t" << Time_D_MV << "\t"
//                    << Time_MV_P << "\t" << Time_PC << "\t"<< Time_TI <<"\n";
...
//
//     vector<vector<double>>Q_L(K,vector<double>(m)),Q_U(K,vector<double>(m));
//     vector<vector<double>>A_L(K,vector<double>(m)),A_U(K,vector<double>(m));
//
//     vector<vector<double>>L_UQ(K,vector<double>(m)),U_LQ(K,vector<double>(m));
//
//
...
//
//       //   // -------------------------------------------------------------------------
//         vector<double> threshold(n1,MAX_VAL);
//         vector<double> threshold_2(n1,MAX_VAL);
//         // if (Time_DTW_TEST) {
//         //     for (int i = 0; i < n1; ++i) {
//         //         vector<double> dtw_values;
//         //         for (int j = 0; j < n2; ++j) {
...
//         //         }
//         //
//         //         sort(dtw_values.begin(), dtw_values.end());
//         //
//         //         size_t index = static_cast<size_t>(dtw_values.size() * 0.8);
//         //
//         //         if (index >= dtw_values.size()) {
//         //             index = dtw_values.size() - 1;
//         //         }
//         //
//         //         threshold[i] = dtw_values[index];
//         //     }
//
//
//         }
...
//                             cb[t]=cb[t+1]+cb[t];
//                         }
//                         // dtw(Test[i], Train[j], K, m, r,threshold_2[i],cb);
...
//       //           start = clock();
//       //           double lb_temp = 0.0;
//       //           vector<vector<double>> lb_record(K,vector<double>(m,0));
//       //           int flag = 0;
//       //           for (int k=0;k<K;k++) {
//       //               for (int t=0;t<m;t++) {
//       //                   lb_temp = 0;
...
//       //                   lower_upper_lemire(new_train[k],m,r,A_L[k],A_U[k]);
//       //                   upper_lemire(lb_record[k],m,r,max_val);
//       //                   for (int t=0;t<m;t++) {
//       //                       double delta = 0;
//       //                       if (new_test[k][i]<A_L[k][t])
//       //                           delta = dist(new_test[k][i],A_L[k][t]);
//       //                       if (new_test[k][i]>A_U[k][t])
//       //                           delta = dist(new_test[k][i],A_U[k][t]);
//       //                       double temp = MAX(0,delta-max_val[t]);
//       //                       cb[t] += temp;
//       //                       lb_mv += temp;
//       //                       if (lb_mv>threshold_2[i]) {flag=1;break;}
//       //                   }
...
//                     start = clock();
//                     int flag = 0;
//                     LB_P_MV_early(new_train,new_test,Q_L,Q_U,new_r,threshold_2[i],flag,lb_p,cb);
//                     if (flag) {
//                         end = clock();
//                         lb_mv_time += (end - start);
//                         continue;
...
void TLB(vector<vector<vector<double>>>& Test, vector<vector<vector<double>>>& Train,string name) {
    ...
    for (int w = 0; w < r_num.size(); w++) {
      ...
      //         sort(dtw_values.begin(), dtw_values.end(),greater<double>());
      //         size_t total_size = dtw_values.size();
      //         size_t start_index = static_cast<size_t>(ceil(total_size * 0.1));
      //         size_t count = total_size - start_index;
      //
      //         double sum = 0.0;
      //         for (size_t k = start_index; k < total_size; ++k) {
      //             sum += dtw_values[k];
      //         }
      //
      //         // 记录阈值
      //         threshold[i] = sum / count;;
      //     }
      //     // end = clock();
      //     // Time_DTW = static_cast<double>(end - start) / CLOCKS_PER_SEC;
      // }
        ...
        if (Time_MV_P_TEST) {
            ...
            for (int i = 0; i < n1; ++i) {
                ...
                for (int j = 0; j < n2; ++j) {
                    ...
                    new_train = transpose(Train[j]);
                    start = clock();
                    lb_p = LB_P_MV(new_train,new_test,Q_L,Q_U,new_r);
                    if (lb_p<threshold_2[i]) {
                        dtw(Test[i], Train[j], K, m, r);
                    }
                    end = clock();
