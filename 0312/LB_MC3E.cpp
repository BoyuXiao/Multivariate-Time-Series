...
vector<vector<double>> Preprocessing_MC3E_PCA(vector<vector<double>>& Q,
    ...
    //把每一组的Q降维并记录下投影矩阵和均值
      ...
      for (int i = 0; i < m; ++i) {
          ...
          //mu and sigma
          // mu_sigma(Q_pca_part,mu_Q_pca[i],sigma_Q_pca[i]);
          split_two_dim(Q_pca_part,mu_Q_pca[i],sigma_Q_pca[i]);
          //U and L
          lower_upper_lemire_pca(num[i],mu_Q_pca[i],sigma_Q_pca[i],U_mu_Q[i],L_mu_Q[i],U_sigma_Q[i],L_sigma_Q[i]);
          //T
...
vector<vector<double>> Preprocessing_MC3E_SORT_strength(vector<vector<double>>& Q,
    ...
    //把每一组的Q降维
      ...
      for (int i = 0; i < m; ++i) {
          ...
          vector<vector<double>> Q_original_part(Q.begin() + start[i], Q.begin() + end[i] + 1);
          //sort split
          splitQ_sort_strength(Q_original_part,Q_sort1,Q_sort2,split_indices_sort[i]);
          // debug------half
          // split_first_and_second(Q_original_part,Q_pca_part);
          // mu_sigma(Q_original_part,Q_original_part[0].size(),num[i],mu_Q_pca[i],sigma_Q_pca[i]);
          ...
          //U and L
          lower_upper_lemire_pca(num[i],mu_Q_sort1[i],mu_Q_sort2[i],U_mu_Q[i],L_mu_Q[i],U_sigma_Q[i],L_sigma_Q[i]);
          //T
...
vector<vector<vector<double>>> Preprocessing_MC3E_PCA_4dim(vector<vector<double>>& Q,
    ...
    //把每一组的Q降维并记录下投影矩阵和均值
      ...
      for (int i = 0; i < m; ++i) {
          ...
          //U and L
          lower_upper_lemire_pca(num[i],mu_Q_pca1[i],sigma_Q_pca1[i],U_mu_Q1[i],L_mu_Q1[i],U_sigma_Q1[i],L_sigma_Q1[i]);
          //T
          ...
          //U and L
          lower_upper_lemire_pca(num[i],mu_Q_pca2[i],sigma_Q_pca2[i],U_mu_Q2[i],L_mu_Q2[i],U_sigma_Q2[i],L_sigma_Q2[i]);
          //T
...
vector<vector<vector<double>>> Preprocessing_MC3E_sort_4dim_strength(vector<vector<double>>& Q,
    ...
    vector<double> corner(2);
      ...
      vector<vector<double>> mu_Q_sort2(m);
      vector<vector<double>> sigma_Q_sort2(m);
    vector<vector<double>> Q_sort_part;
    vector<vector<double>> Q_sort_part1;
    vector<vector<double>> Q_sort_part2;
    ...
    //把每一组的Q降维
      ...
      for (int i = 0; i < m; ++i) {
          ...
          mu_sigma(Q_sort1,mu_Q_sort1[i],sigma_Q_sort1[i]);
          //U and L
          lower_upper_lemire_pca(num[i],mu_Q_sort1[i],sigma_Q_sort1[i],U_mu_Q1[i],L_mu_Q1[i],U_sigma_Q1[i],L_sigma_Q1[i]);
          //T
          pair<vector<double>,vector<double>> Q_part = {mu_Q_sort1[i],sigma_Q_sort1[i]};
          intervalLength = initializeT(T1[i], Q_part, L_mu_Q1[i], U_mu_Q1[i], L_sigma_Q1[i],U_sigma_Q1[i],lambda, num[i]);
          intervalLength_mu1[i]=intervalLength.first;
          intervalLength_sigma1[i]=intervalLength.second;
          ...
          mu_sigma(Q_sort2,mu_Q_sort2[i],sigma_Q_sort2[i]);
          //U and L
          lower_upper_lemire_pca(num[i],mu_Q_sort2[i],sigma_Q_sort2[i],U_mu_Q2[i],L_mu_Q2[i],U_sigma_Q2[i],L_sigma_Q2[i]);
          //T
          Q_part = {mu_Q_sort2[i],sigma_Q_sort2[i]};
          intervalLength = initializeT(T2[i], Q_part, L_mu_Q2[i], U_mu_Q2[i], L_sigma_Q2[i],U_sigma_Q2[i],lambda, num[i]);
          intervalLength_mu2[i]=intervalLength.first;
          intervalLength_sigma2[i]=intervalLength.second;
      }
    ...
    //计算四个dNN预选值(1)
      for (int i = 0; i < m; ++i) {
          ...
          for (int t = 0; t < num[i]; ++t) {
              Qt = {mu_Q_sort1[i][t], sigma_Q_sort1[i][t]};
      ...
      for (int i = 0; i < m; ++i) {
          ...
          for (int t = 0; t < num[i]; ++t) {
              Qt = {mu_Q_sort2[i][t], sigma_Q_sort2[i][t]};
...
vector<vector<vector<double>>> Preprocessing_MC3E_sort_4dim_strength_mumu_sigmasigma(vector<vector<double>>& Q,
    ...
    //把每一组的Q降维
      ...
      for (int i = 0; i < m; ++i) {
          ...
          //前两维
          //U and L
          lower_upper_lemire_pca(num[i],mu_Q_sort1[i],mu_Q_sort2[i],U_mu_Q1[i],L_mu_Q1[i],U_sigma_Q1[i],L_sigma_Q1[i]);
          //T
          pair<vector<double>,vector<double>> Q_part = {mu_Q_sort1[i],mu_Q_sort2[i]};
          intervalLength = initializeT(T1[i], Q_part, L_mu_Q1[i], U_mu_Q1[i], L_sigma_Q1[i],U_sigma_Q1[i],lambda, num[i]);
          intervalLength_mu1[i]=intervalLength.first;
          ...
          //后两维
          //U and L
          lower_upper_lemire_pca(num[i],sigma_Q_sort1[i],sigma_Q_sort2[i],U_mu_Q2[i],L_mu_Q2[i],U_sigma_Q2[i],L_sigma_Q2[i]);
          //T
          Q_part = {sigma_Q_sort1[i],sigma_Q_sort2[i]};
          intervalLength = initializeT(T2[i], Q_part, L_mu_Q2[i], U_mu_Q2[i], L_sigma_Q2[i],U_sigma_Q2[i],lambda, num[i]);
          intervalLength_mu2[i]=intervalLength.first;
          intervalLength_sigma2[i]=intervalLength.second;
      }
    ...
    //计算四个dNN预选值(1)
      ...
      for (int i = 0; i < m; ++i) {
          ...
          for (int t = 0; t < num[i]; ++t) {
              Qt = {sigma_Q_sort1[i][t], sigma_Q_sort2[i][t]};
...
vector<vector<vector<double>>> Preprocessing_MC3E_sigma_4dim_strength(vector<vector<double>>& Q,
    ...
    //把每一组的Q降维
      ...
      for (int i = 0; i < m; ++i) {
          ...
          //split
          vector<vector<double>> Q_original_part(Q.begin() + start[i], Q.begin() + end[i] + 1);
          splitQ_min_metric_heuristic(Q_original_part,Q_sort1,Q_sort2,split_indices_sort[i]);
