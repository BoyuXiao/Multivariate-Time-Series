...
// 使用 Q 的投影矩阵对一维向量 A 进行降维
void reduce_single_A_with_Q_projection(const vector<double>& A,
                                       const Eigen::MatrixXd& projection_matrix,
                                       const Eigen::VectorXd& mean_Q,
                                       vector<double>& A_reduced) {
    int n = A.size();
    int target_dim = projection_matrix.cols();

    A_reduced.clear();
    // 初始化 A_reduced
    A_reduced.resize(target_dim);

    // 转换到 Eigen 矩阵
    Eigen::VectorXd vec_A(n);
    for (int j = 0; j < n; ++j) {
        vec_A(j) = A[j];
    }
    ...
    // 中心化并使用投影矩阵
    vec_A -= mean_Q;
    // Eigen::VectorXd reduced_vec_A = vec_A.transpose() * projection_matrix;
    Eigen::VectorXd reduced_vec_A = projection_matrix.transpose() * vec_A; // 修正后的乘法

    // 填充结果
    for (int j = 0; j < target_dim; ++j) {
        A_reduced[j] = reduced_vec_A(j);
    }
}
