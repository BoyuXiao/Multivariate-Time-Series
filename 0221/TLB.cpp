...
void TLB(vector<vector<vector<double>>>& Test, vector<vector<vector<double>>>& Train,string name) {
    ...
    vector<double> U_mu_Q_pca4, L_mu_Q_pca4, U_sigma_Q_pca4, L_sigma_Q_pca4;
    vector<double> U_mu_QE, L_mu_QE, U_sigma_QE, L_sigma_QE;
    vector<vector<double>> result,resulte,result1,result2,result1_sort,result2_sort,result1_cov,result2_cov,result1_sigma,result2_sigma,result_pca,result_pca4;
    ...
    vector<double> intervalLength_mu_pca4(m);
    vector<double> intervalLength_sigma_pca4(m);
    ...
    for (int w = 0; w < r_num.size(); w++) {
        ...
        for (int i = 0; i <n1; ++i) {
            ...
            U_mu_Q_pca4.clear(), L_mu_Q_pca4.clear(), U_sigma_Q_pca4.clear(), L_sigma_Q_pca4.clear();
            ...
            vector<vector<vector<Cell>>> T_pca4(m);
            //half
            ...
            if (PLUS_SIGMA&&K>=4) {
                ...
                mu_sigma(Q1_sigma, K1_sigma, m, mu_Q1_sigma, sigma_Q1_sigma);
                mu_sigma(Q2_sigma, K2_sigma, m, mu_Q2_sigma, sigma_Q2_sigma);
            ...
            if (PCA&&K>=4) {
                ...
                K_pca4 = Q_pca4[0].size();
                mu_sigma(Q_pca4, K_pca4, m, mu_Q_pca4, sigma_Q_pca4);
                result_pca4 = Preprocessing_MC3E(Q_pca4,mu_Q_pca4,sigma_Q_pca4,U_mu_Q_pca4,L_mu_Q_pca4,U_sigma_Q_pca4,L_sigma_Q_pca4,T_pca4,intervalLength_mu_pca4,intervalLength_sigma_pca4,num,r);
            }
            for (int t=0;t<m;t++) {
                if (mu_Q_pca[i]<mu_Q_pca4[i]) {
                    // cout<<"2<4:"<<mu_Q_pca[i]<<" "<<mu_Q_pca4[i]<<endl;
                    c1++;
                }else {
                    // cout<<"2>4:"<<mu_Q_pca[i]<<" "<<mu_Q_pca4[i]<<endl;
                    c2++;
                }
                // if (sigma_Q_pca[i]<sigma_Q_pca4[i]) {
                //     // cout<<"2<4:"<<sigma_Q_pca[i]<<" "<<sigma_Q_pca4[i]<<endl;
                //     c1++;
                // }else {
                //     // cout<<"2>4:"<<sigma_Q_pca[i]<<" "<<sigma_Q_pca4[i]<<endl;
                //     c2++;
            ...
            for (int j = 0; j < n2; ++j) {
                ...
                if (PLUS_COV&&K>=4) {
                    ...
                    // cout<<K1_correlation<<" "<<K2_correlation<<endl;
                    // if (K1_correlation&&K2_correlation) {
                ...
                if (PLUS_SIGMA&&K>=4) {
                    ...
                    double lb_mc3_plus_1 = LB_MC3E(Q1_sigma, A1_sigma, mu_Q1_sigma,sigma_Q1_sigma,U_mu_Q1_sigma,L_mu_Q1_sigma,U_sigma_Q1_sigma,L_sigma_Q1_sigma,result1_sigma[0],result1_sigma[1],result1_sigma[2],result1_sigma[3],T1_sigma,intervalLength_mu1_sigma,intervalLength_sigma1_sigma,num,K1_sigma);
                    double lb_mc3_plus_2 = LB_MC3E(Q2_sigma, A2_sigma, mu_Q2_sigma,sigma_Q2_sigma,U_mu_Q2_sigma,L_mu_Q2_sigma,U_sigma_Q2_sigma,L_sigma_Q2_sigma,result2_sigma[0],result2_sigma[1],result2_sigma[2],result2_sigma[3],T2_sigma,intervalLength_mu2_sigma,intervalLength_sigma2_sigma,num,K2_sigma);
                ...
                if (PCA&&K>=4) {
                    ...
                    // lb_pca = LB_MC1(Q_pca,A_pca,mu_Q_pca,K_pca,m,r);
                    // lb_pca = LB_MC_NAIVE2(Q_pca,A_pca,mu_Q_pca,sigma_Q_pca,K_pca,m,r);
                    lb_pca = LB_MC_NAIVE1(Q_pca,A_pca,mu_Q_pca,K_pca,m,r);
                    lb_pca_vector[j] = lb_pca;
                }
                if (PCA&&K>=4) {
                    reduce_A_with_Q_projection(Train[j],projection_matrix4,mean_Q4,A_pca4);
                    // lb_pca4 = LB_MC3E(Q_pca4,A_pca4,mu_Q_pca4,sigma_Q_pca4,U_mu_Q_pca4,L_mu_Q_pca4,U_sigma_Q_pca4,L_sigma_Q_pca4,result_pca4[0],result_pca4[1],result_pca4[2],result_pca4[3],T_pca4,intervalLength_mu_pca4,intervalLength_sigma_pca4,num,K_pca4);
                    // lb_pca4 = LB_MC1(Q_pca4,A_pca4,mu_Q_pca4,K_pca4,m,r);
                    // lb_pca4 = LB_MC_NAIVE2(Q_pca4,A_pca4,mu_Q_pca4,sigma_Q_pca4,K_pca4,m,r);
                    lb_pca4 = LB_MC_NAIVE1(Q_pca4,A_pca4,mu_Q_pca4,K_pca4,m,r);
                    lb_pca4_vector[j] = lb_pca4;
                }
                // if (lb_pca>lb_pca4) {
                //     cout<<"2>4:"<<lb_pca<<" "<<lb_pca4<<endl;
                // }else {
                //     cout<<"2<4:"<<lb_pca<<" "<<lb_pca4<<endl;
                // }
